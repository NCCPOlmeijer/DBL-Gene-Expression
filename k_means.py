...
def lees_bestand_en_maak_dictionary(dimensie, cluster_invoer):
    """Deze functie leest het inputbestand and creëert een dictionary.

    Parameter:
    ----------
    dimensie: Integer waarde voor de hoeveelheid dimensies.
    bestandsnaam: Tekstbestand met de CloneID's en relatieve expressiewaarden.
    ...
    waarden.
    """
    with open(cluster_invoer) as file:
...
def verdeling_in_random_clusters(k, dictionary):
    ...
    dictionary: Dictionary met als key de cloneID's en als value een lijst
    van genormaliseerde waarden.
...
def minimale_afstand_tot_cluster(cluster_centrum, data_punten):
    ...
    data_punten: Dictionary met als key de cloneID's en als value een lijst
    van genormaliseerde waarden.
    ...
    for key_cluster, value_cluster in cluster_centrum.items():
        ...
        for waarde in range(0, len(value_cluster)):

            # Bereken de waarden van de afstand tussen de data punten en
            # het cluster centrum
            som += (value_cluster[waarde]-data_punten[waarde])**2
    ...
    for key_indeling, value_indeling in indeling.items():
        ...
        if value_indeling == []:
            ...
            for key1, value1 in indeling.items():
                if len(value1) == 0:
                    continue
                x = random.choice(value1)
                value_indeling.append(x)
...
def bereken_kwadratische_fout(dimensie, data_clusters, norm_val, indeling):
    ...
    norm_val: Dictionary met als key de cloneID's en als value een lijst
    van genormaliseerde waarden.
...
def optimaliseren(cluster_invoer, k, dimensie):
