...
def normaliseer_data(dic):
    ...
    Parameter:
    ----------
    dic: Dictionary met als key CloneID's en als value een lijst van waarden.

    Uitvoer:
    --------
    genormaliseerde_data: Dictionary met als key de cloneID's en als value een
    lijst van genormaliseerde waarden.
    """
    # Creëer een lege dictionary
...
def referentie(dimensie):
    """Functie retourneert een lijst van alle genormaliseerde coördinaten \
...
def dict_inproduct(dimensie, genormaliseerde_data, referentie_genormaliseerd):
    """Deze functie berekent het inproduct tussen alle expressievectoren en \
    de referentievector en geeft een dictionary met alle cloneID's als keys \
    en bijbehorende inproducten als values. Deze inproducten zijn van klein \
    ...
    bijbehorende inproducten als values gesorteerd op alle values van
    klein naar groot.
    """
...
def maken_clusters(k, lijst_inproducten, clone_ID_gesorteerd):
    """Deze functie retourneert een dictionary met alle cloneID's als keys \
    en alle clusternummers als values. Deze values zijn van klein naar \
    groot gesorteerd.
    ...
    for aantal in outliers:
        ...
        # horen
            ...
            for m in range(1, k+1):
                ...
                # Bereken het verschil tussen de outlier en het
                # maximale/minimale inproduct
                inprod_verschil_max = abs(aantal-inprod_max)
                inprod_verschil_min = abs(aantal-inprod_min)
                ...
                # kleiner dan de huidige afstand is, verander de
                # variabele genaamd afstand en het clusternummer
                if afstand == "niet gedefinieerd" or inprod_verschil < afstand:
                    afstand = inprod_verschil
                    cluster = m

            # Voeg de outlier aan de dictionary bij het
            # juiste clusternummer toe
            dict_clusternr_inproduct[cluster].append(aantal)
    ...
    for (key2, value2), (key3, value3) in \
            zip(clone_ID_gesorteerd.items(), dict_inproduct_clusternr.items()):
